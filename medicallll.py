...
async def book(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    user_id_str = str(update.effective_user.id)  # Convert user_id to a string
    pending_appointments_ref.child(user_id_str).set({
        'date': date,
        'time': time
    })
...
async def confirm(update: Update, context: ContextTypes.DEFAULT_TYPE):
    user_id_str = str(update.effective_user.id)  # Convert user_id to a string
    pending_appointment = pending_appointments_ref.child(user_id_str).get()
    ...
    pending_appointments_ref.child(user_id_str).delete()

    await update.message.reply_text(f'Appointment booked for {date} at {time}.')

async def cancel(update: Update, context: ContextTypes.DEFAULT_TYPE):
    user_id_str = str(update.effective_user.id)  # Convert user_id to a string
    if not pending_appointments_ref.child(user_id_str).get():
